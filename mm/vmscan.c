...
static void update_kswapd_threads_node(int nid)
{
	...
	if (kswapd_threads < nr_threads) {
		...
		for (hid = start_idx; hid < (start_idx + increase); hid++) {
			pgdat->mkswapd[hid] = kthread_run_perf_critical(cpu_prime_mask, kswapd, pgdat,
									"kswapd%d:%d", nid, hid);
...
static int multi_kswapd_run(int nid)
{
	...
	for (hid = 1; hid < nr_threads; ++hid) {
		pgdat->mkswapd[hid] = kthread_run_perf_critical(cpu_prime_mask,
								kswapd,
								pgdat, "kswapd%d:%d",
								nid, hid);
		if (IS_ERR(pgdat->mkswapd[hid])) {
...
int kswapd_run(int nid)
{
	...
	pgdat->kswapd = kthread_run_perf_critical(cpu_prime_mask, kswapd,
