...
static int osm_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	...
	for (i = 0; i < parent->osm_table_size; i++) {
		...
		if (core_count == SINGLE_CORE_COUNT || table[i].frequency < 500000)
			table[i].frequency = CPUFREQ_ENTRY_INVALID;
...
if (cpu == 7 && (i - 1) > CPU7_MAX_PERF_IDX)
	per_cpu(clk_boost_pcpu, cpu).max_index = CPU7_MAX_PERF_IDX;
else
